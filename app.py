...
def update_user_token():
    ...
    # Kısa ömürlü token ise otomatik uzat (60 güne çıkar)
    import requests as _req
    ...
    if app_id and app_secret:
        try:
            ...
            if "access_token" in data:
                ...
                flash("✓ Token 60 güne uzatıldı.", "info")
            else:
                flash(f"Token uzatılamadı ({data.get('error',{}).get('message','?')}), kısa token kaydediliyor.", "warning")
        except Exception as e:
            flash(f"Token uzatma hatası: {e}", "warning")
...
if __name__ == "__main__":
    if config.FB_USER_ACCESS_TOKEN and config.FB_PAGES:
        ...
        if bad:
            print(f"⚠ {len(bad)} sayfa yenilenemedi (eski token korundu): "
                  f"{', '.join(p.get('name', p['page_id']) for p in bad)}")
    print("Bot UI çalışıyor → http://127.0.0.1:5000")
